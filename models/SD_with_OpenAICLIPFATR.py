...
class SDModelFATR(PreTrainedModel):
    ...
    def forward(
        ...
        for step in range(self.config.tta.gradient_descent.train_steps):
            ...
            # FATR returns [B, N_tokens, dim], need to pad to match context shape [B, 77, dim]
            # Pad image features from N_tokens (e.g., 66) to 77 to align with text context length
            if image_features.dim() == 3:
                ...
                if N_tokens < target_length:
                    ...
                    # Use mean of existing tokens as padding value (better than zeros)
